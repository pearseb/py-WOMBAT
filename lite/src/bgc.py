...
def compute_sourcessinks(phy_mu, phy_lmort, phy_qmort, phy_dfeupt,
                         zoo_zoores, zoo_qmort, zoo_grzphy, zoo_grzdet, det_remin, 
    ...
    ddt_no3 = (
        (phy_lmort + zoo_zoores + det_remin +
         (1.0 - BGC.zoo_assim) * BGC.zoo_excre * (zoo_grzphy + zoo_grzdet)) / BGC.phy_CN
        - phy_mu * phy_loc / BGC.phy_CN
    )

    ddt_dfe = (
        (phy_lmort * phyFeC + zoo_zoores * zooFeC + det_remin * detFeC +
         (1.0 - BGC.zoo_assim) * BGC.zoo_excre * (zoo_grzphy * phyFeC + zoo_grzdet * detFeC))
    ...
    ddt_zoo = (zoo_grzphy + zoo_grzdet) * BGC.zoo_assim - (zoo_zoores + zoo_qmort)

    ddt_zoofe = (
        (zoo_grzphy * phyFeC + zoo_grzdet * detFeC) * BGC.zoo_assim
        - (zoo_zoores + zoo_qmort) * zooFeC
    )

    ddt_det = (
        (zoo_grzphy + zoo_grzdet) * (1.0 - BGC.zoo_assim) * (1.0 - BGC.zoo_excre)
        + phy_qmort + zoo_qmort - det_remin - zoo_grzdet
    )

    ddt_detfe = (
        (zoo_grzphy * phyFeC + zoo_grzdet * detFeC) * (1.0 - BGC.zoo_assim) * (1.0 - BGC.zoo_excre)
